...
export class News extends Component {
   ...
   async componentDidMount(){
      ...
      this.setState({articles: parsedData.artiles})
  ...
  render() {
    return (
      <div className='container my-3'>
        ...
        <div className="row">
        {this.state.artiles.map((element)=>{
        
        return   <div className="col-md-4" key={element.url}>
         <NewsaItem title={element.title?element.title.slice(0,45):""} discription={element.description?element.description.slice(0,88):""} imageUrl={element.urlToImage} newsUrl={element.url}></NewsaItem>
         </div>
        })}
        </div>
